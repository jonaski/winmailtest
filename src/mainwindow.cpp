...
void MainWindow::TestMAPILogon() {
  ...
  LPMAPILOGON mapi_logon = reinterpret_cast<LPMAPILOGON>(GetProcAddress(mapi_lib_, "MAPILogon"));
  if (!mapi_logon) {
...
void MainWindow::TestMAPISendMail() {
  ...
  LPMAPISENDMAIL mapi_sendmail = reinterpret_cast<LPMAPISENDMAIL>(GetProcAddress(mapi_lib_, "MAPISendMail"));
  if (!mapi_sendmail) {
